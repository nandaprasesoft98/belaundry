...
| Headers:\n$headers''');
    ...
    Dio dio = ModifiedDio(Dio(baseOptions));
...
class _DioHttpClientOptionsImpl {
  Map<String, dynamic> createTokenHeader(String tokenWithBearer) {
    return <String, dynamic> {
      if (!tokenWithBearer.isEmptyString) "token": tokenWithBearer,
...
class ModifiedDio implements Dio {
  ...
  ModifiedDio(this._wrappedDio);

  Options get _optionsWithTokenHeader => _DioHttpClientOptions.createOptionsWithTokenHeader(LoginHelper.getTokenWithBearer().result);
